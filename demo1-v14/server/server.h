...
public slots:
    int handle_client_request(QByteArray request,QByteArray &ret,void *addr)
    {
        ...
        case Protocol::SET_CONFIG:
        {
            ...
            obj_2_cfg(config);
            save_cfg();
            camera_manager->set_config(cfg.profile);
            break;
        }
        ...
        case Protocol::MOD_CAMERA_ALG:
        {
            ...
            camera_manager->modify_camera(index,alg,CameraManager::MODIFY_ALG);
            QJsonObject obj;
            camera_manager->get_config(obj);
            QJsonObject config;
...
private:
    ...
    void obj_2_cfg(QJsonObject config)
    {
        cfg.server_name=config["device_name"].toString();
        cfg.profile=config["profile"].toObject();
    }
    QJsonObject cfg_2_obj()
    {
        QJsonObject config;

        config["device_name"] = cfg.server_name;
